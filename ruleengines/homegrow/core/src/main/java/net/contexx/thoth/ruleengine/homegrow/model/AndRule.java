...
public class AndRule<IdentityType> implements Rule<IdentityType> {
    public AndRule(List<Rule<IdentityType>> rules) {
        this.rules = rules;
    }

    public AndRule(Rule<IdentityType>...rules) {
    ...
    public EvaluationResult<IdentityType> evaluate(Attributable attributable, ExecutionContext<IdentityType> context) {
        ...
        return new EvaluationResult<>(this, nestedResults.parallelStream().allMatch(EvaluationResult::isAllowed), nestedResults){
            @Override
            public void prittyPrint(StringBuilder output, String prefix) {
                output.append(prefix).append("AND {\n");
